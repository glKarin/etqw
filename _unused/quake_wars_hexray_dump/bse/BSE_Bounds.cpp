double __stdcall rvParticleTemplate::GetMaxParmValue(rvParticleParms *spawn, rvParticleParms *death, rvEnvParms *envelope)
{
  double result; // st7
  idBounds sbounds; // [esp+4h] [ebp-34h]
  idBounds dbounds; // [esp+1Ch] [ebp-1Ch]
  float v6; // [esp+34h] [ebp-4h]

  rvParticleParms::GetMinsMaxs(spawn, (idVec3 *)((char *)sbounds.b + 4), (idVec3 *)((char *)&sbounds.b[1] + 4));
  if ( rvEnvParms::GetMinMax(envelope, (float *)&sbounds, (float *)&spawn) )
  {
    sbounds.b[0].y = sbounds.b[0].y * sbounds.b[0].x;
    sbounds.b[0].z = sbounds.b[0].z * sbounds.b[0].x;
    sbounds.b[1].x = sbounds.b[0].x * sbounds.b[1].x;
    sbounds.b[1].y = sbounds.b[1].y * *(float *)&spawn;
    sbounds.b[1].z = sbounds.b[1].z * *(float *)&spawn;
    dbounds.b[0].x = *(float *)&spawn * dbounds.b[0].x;
    rvParticleParms::GetMinsMaxs(death, (idVec3 *)((char *)dbounds.b + 4), (idVec3 *)((char *)&dbounds.b[1] + 4));
    envelope = (rvEnvParms *)&dbounds.b[0].y;
    death = (rvParticleParms *)&sbounds.b[0].y;
    dbounds.b[0].y = dbounds.b[0].y * sbounds.b[0].x;
    dbounds.b[0].z = dbounds.b[0].z * sbounds.b[0].x;
    dbounds.b[1].x = sbounds.b[0].x * dbounds.b[1].x;
    dbounds.b[1].y = dbounds.b[1].y * *(float *)&spawn;
    dbounds.b[1].z = dbounds.b[1].z * *(float *)&spawn;
    v6 = *(float *)&spawn * v6;
    sbounds.b[0].y = fminf(sbounds.b[0].y, dbounds.b[0].y);
    sbounds.b[0].z = fminf(sbounds.b[0].z, dbounds.b[0].z);
    sbounds.b[1].x = fminf(sbounds.b[1].x, dbounds.b[1].x);
    sbounds.b[1].y = fmaxf(sbounds.b[1].y, dbounds.b[1].y);
    sbounds.b[1].z = fmaxf(sbounds.b[1].z, dbounds.b[1].z);
    dbounds.b[0].x = fmaxf(dbounds.b[0].x, v6);
  }
  *(float *)&spawn = sbounds.b[1].x * sbounds.b[1].x + sbounds.b[0].y * sbounds.b[0].y + sbounds.b[0].z * sbounds.b[0].z;
  *(float *)&spawn = sqrt(*(float *)&spawn);
  sbounds.b[0].x = *(float *)&spawn;
  *(float *)&spawn = dbounds.b[0].x * dbounds.b[0].x + sbounds.b[1].y * sbounds.b[1].y + sbounds.b[1].z * sbounds.b[1].z;
  *(float *)&spawn = sqrt(*(float *)&spawn);
  result = *(float *)&spawn;
  if ( sbounds.b[0].x >= (double)*(float *)&spawn )
    result = sbounds.b[0].x;
  return result;
}
void __stdcall rvParticleTemplate::EvaluateSimplePosition(idVec3 *pos, float time, float lifeTime, idVec3 *initPos, idVec3 *velocity, idVec3 *acceleration, idVec3 *friction)
{
  idVec3 *v7; // esi
  ...
  timea = time * (0.5 * time);
  v7->x = acceleration->x * timea + v7->x;
  v7->y = acceleration->y * timea + v7->y;
  v7->z = acceleration->z * timea + v7->z;
  velocitya = (lifeTime - timea) / lifeTime;
  velocityb = exp(velocitya);
  timeb = (velocityb - 1.0) * (0.3333333432674408 * timea) * timea;
  v7->x = friction->x * timeb + v7->x;
  v7->y = friction->y * timeb + v7->y;
  v7->z = timeb * friction->z + v7->z;
}
double __thiscall rvParticleTemplate::GetFurthestDistance(rvParticleTemplate *this)
{
  ...
  float v82; // xmm5_4
  double v83; // st7
  idVec3 pos; // [esp+24h] [ebp-124h]
  float max; // [esp+34h] [ebp-114h]
  idBounds result; // [esp+38h] [ebp-110h]
  idBounds accel; // [esp+50h] [ebp-F8h]
  idBounds friction; // [esp+68h] [ebp-E0h]
  idBounds spawn; // [esp+80h] [ebp-C8h]
  idBounds velocity; // [esp+98h] [ebp-B0h]
  idBounds offset; // [esp+B0h] [ebp-98h]
  ...
  v1 = this;
  rvParticleParms::GetMinsMaxs(
    this->mpSpawnPosition,
    (idVec3 *)((char *)spawn.b + 4),
    (idVec3 *)((char *)&spawn.b[1] + 4));
  rvParticleParms::GetMinsMaxs(
    v1->mpSpawnVelocity,
    (idVec3 *)((char *)velocity.b + 4),
    (idVec3 *)((char *)&velocity.b[1] + 4));
  rvParticleParms::GetMinsMaxs(
    v1->mpSpawnAcceleration,
    (idVec3 *)((char *)accel.b + 4),
    (idVec3 *)((char *)&accel.b[1] + 4));
  rvParticleParms::GetMinsMaxs(
    v1->mpSpawnFriction,
    (idVec3 *)((char *)friction.b + 4),
    (idVec3 *)((char *)&friction.b[1] + 4));
  rvParticleParms::GetMinsMaxs(
    v1->mpSpawnOffset,
    (idVec3 *)((char *)offset.b + 4),
    (idVec3 *)((char *)&offset.b[1] + 4));
  accel.b[0].x = 0.0;
  result.b[1].z = 0.0;
  v103 = &pos.y;
  result.b[1].y = 0.0;
  ...
  v102 = &pos.y;
  v104 = &result.b[0].y;
  v92 = COERCE_FLOAT((idVec3 *)((char *)&pos + 4));
  LODWORD(result.b[0].x) = (char *)&result + 4;
  do
  {
    max = (double)SLODWORD(max) * v1->mDuration.y * 0.125;
...
double __thiscall rvSegmentTemplate::CalculateBounds(rvSegmentTemplate *this)
{
...
double __thiscall rvDeclEffect::CalculateBounds(rvDeclEffect *this)
{
  ...
  if ( this->mSegmentTemplates.num > 0 )
  {
    ...
    do
    {
      v7 = rvSegmentTemplate::CalculateBounds(&v1->mSegmentTemplates.list[v3]);
  ...
  return (float)_ceil(segSize);
}
int dynamic_initializer_for__singletonLock___112()
{
  sdLock::sdLock(&singletonLock_112);
  return atexit(dynamic_atexit_destructor_for__singletonLock___112);
}
void __cdecl dynamic_atexit_destructor_for__singletonLock___112()
{
  sdLock::~sdLock(&singletonLock_112);
}
