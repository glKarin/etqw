...
public:
    ...
    void					UpdateLatched( void )
    {
        internalVar->InternalUpdateLatched();
    }
    ...
    void					SetGuiLocked( void )
    {
        internalVar->flags |= CVAR_GUILOCKED;
    }
    void					ClearGuiLocked( void )
    {
        internalVar->flags &= ~CVAR_GUILOCKED;
    }
    ...
    {
        internalVar->InternalSetString( value, true, true );
    }
    void					SetBool( const bool value )
    {
        internalVar->InternalSetBool( value, true, true );
    }
    void					SetInteger( const int value )
    {
        internalVar->InternalSetInteger( value, true, true );
    }
    void					SetFloat( const float value )
    {
        internalVar->InternalSetFloat( value, true, true );
    }
...
private:
    ...
    virtual void			InternalSetString( const char *newValue, bool force, bool clearLatched ) {}
    virtual void			InternalSetBool( const bool newValue, bool force, bool clearLatched ) {}
    virtual void			InternalSetInteger( const int newValue, bool force, bool clearLatched ) {}
    virtual void			InternalSetFloat( const float newValue, bool force, bool clearLatched ) {}
...
public:
    ...
    virtual void			ResetFlaggedVariables( int flags, int ignoreFlags = 0 ) = 0;
                             ...
                             float valueMin, float valueMax, const char **valueStrings, argCompletion_t valueCompletion )
{
    ...
    if ( staticVars != (idCVar *)((1 << sizeof(idCVar *)) - 1) ) {
...
ID_INLINE void idCVar::RegisterStaticVars( void )
{
    if ( staticVars != (idCVar *)((1 << sizeof(idCVar *)) - 1) ) {
        ...
        staticVars = (idCVar *)((1 << sizeof(idCVar *)) - 1);
    }
}
