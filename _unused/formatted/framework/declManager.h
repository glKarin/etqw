...
public:
    ...
    void						InvalidateAndDiscard( void )
    {
        base->InvalidateAndDiscard();
    }
    ...
    void						SetBinarySource( const byte* source, int length )
    {
        base->SetBinarySource( source, length );
    }

    // Retrieve binary source data. FreeSourceBuffer must be called when the client is finished with source.
    void						GetBinarySource( byte*& source, int& length ) const
    {
        base->GetBinarySource( source, length );
    }

    // Frees the buffer allocated by a previous call to GetBinarySource.
    void						FreeSourceBuffer( byte* source ) const
    {
        base->FreeSourceBuffer( source );
    }

    bool						HasBinaryBuffer() const
    {
        return base->HasBinaryBuffer();
    }
    ...
    void						ReParse( void ) const
    {
        const_cast< idDeclBase* >( base )->ReParse();
    }
...
public:
    ...
    {
        return base->GetFileOffset();
    }
    virtual int					GetFileLength( void ) const
    {
        return base->GetFileLength();
    }
    ...
    {
        return base->GetIncludeDependencies();
    }
    virtual const idStrList*	GetFileLevelIncludeDependencies() const
    {
        return base->GetFileLevelIncludeDependencies();
    }
...
{
public:
    DECLTYPE*				New( const char* name, const char* file ) const
    {
        return reinterpret_cast< DECLTYPE* >(const_cast< idDecl* >( CreateNewDecl( name, file ) ));
    }
    DECLTYPE*				operator[]( const char* name ) const
    {
        return reinterpret_cast< DECLTYPE * >(const_cast< idDecl* >( Find( name, false ) ));
    }
    DECLTYPE*				operator[]( int index ) const
    {
        return reinterpret_cast< DECLTYPE * >(const_cast< idDecl* >( FindByIndex( index, true ) ));
    }
    DECLTYPE*				SafeIndex( int index ) const
    {
        return ( index < 0 || index >= Num() ) ? NULL : reinterpret_cast< DECLTYPE* >( const_cast< idDecl* >( FindByIndex( index, true ) ) );
    }
    DECLTYPE*				LocalFind( const char* name, bool makeDefault = true ) const
    {
        return reinterpret_cast< DECLTYPE* >( const_cast< idDecl* >( Find( name, makeDefault ) ) );
    }
    DECLTYPE*				LocalFindByIndex( int index, bool forceParse = true ) const
    {
        return reinterpret_cast< DECLTYPE* >( const_cast< idDecl* >( FindByIndex( index, forceParse ) ) );
    }
};
