...
public:
    ...
    {
        return sizeof(sdDeclImposter);
    }

    static void				CacheFromDict( const idDict& dict );

    // New
    const sdImposterSubImage &	GetSubImage( int index ) const
    {
        return info.images[ index ];
    }
    sdImposterSubImage &		GetSubImage( int index )
    {
        return info.images[ index ];
    }
    ...
    float						GetScreenScale( void ) const
    {
        return info.screenScale;
    }
    int							GetNumAngles( void ) const
    {
        return info.numAngles;
    }

    const imposterInfo_t&		GetInfo( void ) const
    {
        return info;
    }
    ...
    void						ClearSubImages( void )
    {
        info.images.Clear();
    }
    void						AddSubImage( const sdImposterSubImage &image )
    {
...
public:
    ...
    virtual size_t			Size( void ) const
    {
        return sizeof(sdDeclImposterGenerator);
    }

    // New
    const char *			GetSourceModelName( void ) const
    {
        return sourceModel.c_str();
    }
    const char *			GetOutputTextureName( void ) const
    {
        return outputTexture.c_str();
    }
    bool					UseVertexColors( void ) const
    {
        return vertexColor;
    }
    int						GetNumAngles( void ) const
    ...
    int						GetTileSizeX( void ) const
    {
        return tileSize[0];
    }
    int						GetTileSizeY( void ) const
    {
        return tileSize[1];
    }
    bool					GetNoBump( void ) const
    {
        return noBump;
    }
