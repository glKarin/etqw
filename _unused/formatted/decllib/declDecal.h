...
public:
    ...
    {
        return sizeof(sdDeclDecal);
    }
    static  void			CacheFromDict( const idDict& dict );

    // New for this decl
    idVec4					GetStartColor( void ) const
    {
        return startColor;
    }
    idVec4					GetEndColor( void ) const
    {
        return endColor;
    }
    ...
    const sdBounds2D&		GetImage( int index ) const
    {
        return images[index];
    }

    float					GetSpawnSize( void ) const
    {
        return minSize + idRandom::StaticRandom().RandomFloat() * sizeDiff;
    }
private:
    idVec4	startColor;
    idVec4	endColor;
