...
public:
    ...
    {
        flags.validationEnabled = validationEnabled;
    }
    void SetCallbackEnabled( bool callbackEnabled )
    {
        flags.callbackEnabled = callbackEnabled;
    }
...
public:
    ...
    bool IsValid() const
    {
        return value.baseValue != NULL;
    }
    bool IsReadOnly() const
    {
        return value.baseValue->IsReadOnly();
    }
    const char* TypeToString() const
    {
        return TypeToString( valueType );
    }
    ePropertyType GetValueType() const
    {
        return valueType;
    }
    ...
    void NumAttached( int& onChange, int& onValidate ) const
    {
        switch( valueType ) {
        case PT_INT:
            value.intValue->	NumAttached( onChange, onValidate );
            break;
        case PT_FLOAT:
            value.floatValue->	NumAttached( onChange, onValidate );
            break;
        case PT_BOOL:
            value.boolValue->	NumAttached( onChange, onValidate );
            break;
        case PT_STRING:
            value.stringValue->	NumAttached( onChange, onValidate );
            break;
        case PT_WSTRING:
            value.wstringValue->NumAttached( onChange, onValidate );
            break;
        case PT_VEC2:
            value.vec2Value->	NumAttached( onChange, onValidate );
            break;
        case PT_VEC3:
            value.vec3Value->	NumAttached( onChange, onValidate );
            break;
        case PT_VEC4:
            value.vec4Value->	NumAttached( onChange, onValidate );
            break;
        case PT_COLOR3:
            value.color3Value->	NumAttached( onChange, onValidate );
            break;
        case PT_COLOR4:
            value.color4Value->	NumAttached( onChange, onValidate );
            break;
        case PT_ANGLES:
            value.anglesValue-> NumAttached( onChange, onValidate );
            break;
        }
    }
    ...
    void RemoveOnChangeHandler( CallbackHandle handle )
    {
        switch( valueType ) {
        case PT_INT:
            value.intValue->	RemoveOnChangeHandler( handle );
            break;
        case PT_FLOAT:
            value.floatValue->	RemoveOnChangeHandler( handle );
            break;
        case PT_BOOL:
            value.boolValue->	RemoveOnChangeHandler( handle );
            break;
        case PT_STRING:
            value.stringValue->	RemoveOnChangeHandler( handle );
            break;
        case PT_WSTRING:
            value.wstringValue->RemoveOnChangeHandler( handle );
            break;
        case PT_VEC2:
            value.vec2Value->	RemoveOnChangeHandler( handle );
            break;
        case PT_VEC3:
            value.vec3Value->	RemoveOnChangeHandler( handle );
            break;
        case PT_VEC4:
            value.vec4Value->	RemoveOnChangeHandler( handle );
            break;
        case PT_COLOR3:
            value.color3Value->	RemoveOnChangeHandler( handle );
            break;
        case PT_COLOR4:
            value.color4Value->	RemoveOnChangeHandler( handle );
            break;
        case PT_ANGLES:
            value.anglesValue-> RemoveOnChangeHandler( handle );
            break;
        }
    }
