...
{
    if ( numBits > ( maxSize << 3 ) - GetNumBitsWritten() ) {
        if ( !allowOverflow ) {
            idLib::common->Error( "idBitMsg: overflow without allowOverflow set" );
        }
        if ( numBits > ( maxSize << 3 ) ) {
            idLib::common->Error( "idBitMsg: %i is > full message size", numBits );
        }
...
byte *idBitMsg::GetByteSpace( int length )
{
    ...
    if ( length <= 0 ) {
        idLib::common->FatalError( "idBitMsg::GetByteSpace: length must be greater than zero ( was %d )", length );
    }
...
void idBitMsg::WriteBits( int value, int numBits )
{
    ...
    // (since I'm merely propagating the check and I won't closely test the networking at this point to check for problems, making this a warning)
        ...
        if ( numBits > 0 ) {
            if ( value > ( 1 << numBits ) - 1 ) {
                ...
                if ( now - last_dump > 1000 ) {
                    idLib::common->Printf( idLib::sys->GetCurCallStackStr( 16 ) );
                    last_dump = now;
                }
            } else if ( value < 0 ) {
                idLib::common->Warning( "idBitMsg::WriteBits: value overflow %d %d", value, numBits );
...
/*
===============
idBitMsg::EnsureByteBoundary
===============
*/
void idBitMsg::EnsureByteBoundary( void )
{
    if ( writeBit > 0 ) {
        // skip over to the next boundary
        // the byte was already initialized to 0 so we don't need to write any padding
        curSize++;
        writeBit = 0;
    }
}
...
/*
================
idBitMsg::WriteDeltaData
================
*/
void idBitMsg::WriteDeltaData( const void* oldValue, const void* newValue, int size )
{
    if ( !memcmp( oldValue, newValue, size ) ) {
        WriteBits( 0, 1 );
        return;
    }
    WriteBits( 1, 1 );
    WriteData( newValue, size );
}
...
================
*/
void idBitMsg::ReadDeltaData( const void* oldValue, void* newValue, int size ) const
{
    if ( ReadBits( 1 ) ) {
        ReadData( newValue, size );
    }
