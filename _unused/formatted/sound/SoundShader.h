...
public:
    ...
    bool					IsPrivateSound( void ) const
    {
        return( !!( parms.soundShaderFlags & SSF_PRIVATE_SOUND ) );
    }
    bool					IsAntiPrivateSound( void ) const
    {
        return( !!( parms.soundShaderFlags & SSF_ANTI_PRIVATE_SOUND ) );
    }
    bool					IsNoOcclusion( void ) const
    {
        return( !!( parms.soundShaderFlags & SSF_NO_OCCLUSION ) );
    }
    bool					IsGlobal( void ) const
    {
        return( !!( parms.soundShaderFlags & SSF_GLOBAL ) );
    }
    bool					IsOmnidirectional( void ) const
    {
        return( !!( parms.soundShaderFlags & SSF_OMNIDIRECTIONAL ) );
    }
    bool					IsLooping( void ) const
    {
        return( !!( parms.soundShaderFlags & SSF_LOOPING ) );
    }
    bool					IsPlayOnce( void ) const
    {
        return( !!( parms.soundShaderFlags & SSF_PLAY_ONCE ) );
    }
    bool					IsOnDemand() const
    {
        return onDemand;
    }
    bool					IsLowPriority( void ) const
    {
        return lowPriority;
    }
    ...
    idSoundSample*			GetEntry( int index ) const
    {
        return( entries[index] );
    }

    int						GetSpeakerMask() const
    {
        return speakerMask;
    }
    float					GetLeadinVolume() const
    {
        return leadinVolume;
    }
