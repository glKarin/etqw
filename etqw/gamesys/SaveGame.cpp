...
#ifdef _ETQW //karin: spawnID is entityNum
	WriteInt( renderEntity.spawnID );
#else
...
#ifdef _ETQW //karin: rename to material
	WriteMaterial( renderLight.material );
#else
...
void idRestoreGame::ReadRenderEntity( renderEntity_t &renderEntity ) {
...
#ifdef _ETQW //karin: rename to spawnID
	ReadInt( renderEntity.spawnID );
#else
	...
	for( i = 0; i < MAX_RENDERENTITY_GUI; i++ ) {
#ifdef _ETQW
		guiHandle_t handleUnused;
		ReadUserInterface( handleUnused );
#else
...
#ifdef _ETQW //karin: rename to material
	ReadMaterial( renderLight.material );
#else
...
void idRestoreGame::ReadFunction( const function_t*& function ) {
	...
	if ( funcname.Length() ) {
#ifdef _ETQW //karin: ->
		function = (const function_t *)gameLocal.program->FindFunction( funcname );
#else
