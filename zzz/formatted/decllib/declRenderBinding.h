...
public:
    ...
    {
        return sizeof( sdDeclRenderBinding );
    }
    virtual void					FreeData();

    bindingType_t					GetBindingType() const
    {
        return type;
    }
    ...
    float*							GetDefaultVector() const
    {
        return const_cast< float* >( defaults.vector );
    }
    idImage*						GetDefaultImage() const
    {
        return const_cast< idImage* >( defaults.texture.image );
    }
    int								GetDefaultAttribIndex() const
    {
        return defaults.attrib;
    }
    ...
    idVec3&							GetVec3() const
    {
        return *(idVec3 *)data.vector;
    }
    idVec4&							GetVec4() const
    {
        return *(idVec4 *)data.vector;
    }
    textureDepth_t					GetTextureDepth() const
    {
        return data.texture.defaultDepth;
    }
    cubeFiles_t						GetCubeMap() const
    {
        return data.texture.defaultCubeMap;
    }

    int								Infrequent() const
    {
        return infrequent;
    }
    void							SetInfrequentIndex( const int index ) const
    {
        infrequent = index;
    }

    // Only for renderer use, the rest of the engine HANDS OFF THE GOODS!
    void							SetEvaluator( evaluatorCallback_t evaluator ) const
    {
        this->evaluator = evaluator;
    }
    void							ClearEvaluator() const
    {
        this->evaluator = NoOpEvaluator;
    }
    void							Evaluate() const;
