...
public:
    ...
    {
        return ( numStages > 0 || entityGui != 0 );
    }
    ...
    bool				ShadowsCastOnlyFromStaticObjects( void ) const
    {
        return TestMaterialFlag( MF_SHADOWSCASTONLYFROMSTATICOBJECTS );
    }
    ...
    {
        return ( materialFlags & (MF_NOSELFSHADOW|MF_NOSHADOWS|MF_RECEIVESLIGHTINGONBACKSIDES) ) != 0;
    }
    ...
    const idMaterial *	GetBackSideMaterial( void ) const
    {
    ...
    {
        return ( entityGui || deform != DFRM_NONE || sort == SS_SUBVIEW ||
    ...
    bool				IsOccluder( void ) const
    {
        return TestMaterialFlag( MF_OCCLUSION_OCCLUDE );
    }
    bool				UseOcclusionQuery( void ) const
    {
        return TestMaterialFlag( MF_OCCLUSION_QUERY );
    }
    ...
    // get portal flags
    const int			GetPortalFlags() const
    {
        return portalFlags;
    }
    ...
    void				SetPolygonOffset( float po )
    {
        polygonOffset = po;
    }
    ...
    const class sdDeclSurfaceType*		GetSurfaceType( void ) const
    {
        return surfaceTypeDecl;
    }
    const class sdDeclSurfaceTypeMap*	GetSurfaceTypeMapDecl( void ) const
    {
        return surfaceTypeMapDecl;
    }
    const class sdSurfaceTypeMap*		GetSurfaceTypeMap( void ) const
    {
        return surfaceTypeMap;
    }
    const idVec3&		GetSurfaceColor( void ) const
    {
        return surfaceColor;
    }

    ID_INLINE int		GetGpuSpec( void ) const
    {
        return gpuSpec;
    }
    ...
    const subviewInfo_t&	GetSubviewInfo() const
    {
        return subviewInfo;
    }
    const sdParameterInfo&	GetParameterInfo( int index ) const
    {
        return parameterInfo[index];    //For visual shader tweaking tools
    }
    unsigned int			GetBreakpoints() const
    {
        return breakpointFlags;
    }

    const float			GetSlopTexCoordMod() const
    {
        return slopTexCoordMod;
    }
