...
ID_INLINE void sdFromString( sdProperty& value, const char* str )
{
    ...
    case PT_STRING:
        *value.value.stringValue = str;
        break;
    case PT_WSTRING:
        *value.value.wstringValue = va( L"%hs", str );
        break;
    case PT_INT:
        sdFromString( *value.value.intValue,		sdEnforceNumericFormat( sdPropertyTraits< int >::dimension, str ) );
        break;
    case PT_FLOAT:
        sdFromString( *value.value.floatValue,		sdEnforceNumericFormat( sdPropertyTraits< float >::dimension, str ) );
        break;
    case PT_BOOL:
        sdFromString( *value.value.boolValue, 		sdEnforceNumericFormat( sdPropertyTraits< bool >::dimension, str ) );
        break;
    case PT_VEC2:
        sdFromString( *value.value.vec2Value, 		sdEnforceNumericFormat( sdPropertyTraits< idVec2 >::dimension, str ) );
        break;
    case PT_VEC3:
        sdFromString( *value.value.vec3Value, 		sdEnforceNumericFormat( sdPropertyTraits< idVec3 >::dimension, str ) );
        break;
    case PT_VEC4:
        sdFromString( *value.value.vec4Value, 		sdEnforceNumericFormat( sdPropertyTraits< idVec4 >::dimension, str ) );
        break;
    case PT_COLOR3:
        sdFromString( *value.value.color3Value, 	sdEnforceNumericFormat( sdPropertyTraits< sdColor3 >::dimension, str ) );
        break;
    case PT_COLOR4:
        sdFromString( *value.value.color4Value, 	sdEnforceNumericFormat( sdPropertyTraits< sdColor4 >::dimension, str ) );
        break;
    case PT_ANGLES:
        sdFromString( *value.value.anglesValue, 	sdEnforceNumericFormat( sdPropertyTraits< idAngles >::dimension, str ) );
        break;
    default:
...
ID_INLINE idStr sdToString( const sdProperty& value )
{
    ...
    switch( value.GetValueType() ) {
    case PT_STRING:
        return value.value.stringValue->GetValue();
    case PT_WSTRING:
        return va( "%ls", value.value.wstringValue->GetValue().c_str() );
    case PT_INT:
        return sdToString( *value.value.intValue	);
    case PT_FLOAT:
        return sdToString( *value.value.floatValue	);
    case PT_BOOL:
        return sdToString( *value.value.boolValue 	);
    case PT_VEC2:
        return sdToString( *value.value.vec2Value 	);
    case PT_VEC3:
        return sdToString( *value.value.vec3Value 	);
    case PT_VEC4:
        return sdToString( *value.value.vec4Value 	);
    case PT_COLOR3:
        return sdToString( *value.value.color3Value );
    case PT_COLOR4:
        return sdToString( *value.value.color4Value );
    case PT_ANGLES:
        return sdToString( *value.value.anglesValue );
    default:
        assert( !"sdFromString:: Invalid property type" );
    }
    return "";
