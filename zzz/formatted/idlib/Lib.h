...
ID_INLINE void		SwapLittleShort( short &c )
{
    c = LittleShort( c );
}
ID_INLINE void		SwapLittleUnsignedShort( unsigned short &c )
{
    c = LittleShort( c );
}
ID_INLINE void		SwapLittleInt( int &c )
{
    c = LittleLong( c );
}
ID_INLINE void		SwapLittleUnsignedInt( unsigned int &c )
{
    c = LittleLong( c );
}
ID_INLINE void		SwapLittleFloat( float &c )
{
    c = LittleFloat( c );
}
template<class type>
ID_INLINE void		SwapLittleFloatClass( type &c )
{
    for ( int i = 0; i < c.GetDimension(); i++ ) {
        c.ToFloatPtr()[i] = LittleFloat( c.ToFloatPtr()[i] );
    }
}
