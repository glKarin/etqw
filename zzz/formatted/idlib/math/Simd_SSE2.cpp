...
============
*/
void SSE2_Cross( const idVec4 &v1, const idVec4 &v2, idVec4 &result )
{
    __asm {
        mov			esi, v1
        mov			edi, v2
        mov			ecx, result
        pshufd		xmm1, [edi], R_SHUFFLE_D( 1, 2, 0, 3 )		// xmm1 =        v2.y,        v2.z,        v2.x
        mulps		xmm1, [esi]									// xmm1 = v1.x * v2.y, v1.y * v2.z, v1.z * v2.x
        pshufd		xmm0, [esi], R_SHUFFLE_D( 1, 2, 0, 3 )		// xmm0 =        v1.y,        v1.z,        v1.x
...
============
*/
void SSE2_Dot( const idVec4 &v1, const idVec4 &v2, float &result )
{
    __asm {
        mov			esi, v1
        mov			edi, v2
        ...
        movhlps		xmm1, xmm0
        addps		xmm0, xmm1
        pshufd		xmm1, xmm0, R_SHUFFLE_D( 0, 1, 0, 0 )
        addss		xmm0, xmm1
        movss		[ecx], xmm0
    }
}
...
============
*/
void VPCALL idSIMD_SSE2::TransformVerts( idDrawVert *verts, const int numVerts, const idJointMat *joints, const idVec4 *base, const jointWeight_t *weights, const int numWeights )
{
...
/*
============
idSIMD_SSE2::TransformShadowVerts
============
*/
void VPCALL idSIMD_SSE2::TransformShadowVerts( idDrawVert *verts, const int numVerts, const idJointMat *joints, const idDrawVert *base, const jointWeight_t *weights, const int numWeights )
{
    assert_16_byte_aligned( joints );
    assert_16_byte_aligned( base );
...
============
*/
void VPCALL idSIMD_SSE2::TransformShadowVerts( idDrawVert *verts, const int numVerts, const idJointMat *joints, const idDrawVert *base, const short *weights, const int numWeights )
{
    assert_16_byte_aligned( joints );
    assert_16_byte_aligned( base );
...
============
*/
void VPCALL idSIMD_SSE2::TransformShadowVerts( shadowCache_t *verts, const int numVerts, const idJointMat *joints, const idDrawVert *base, const short *weights, const int numWeights )
{
    assert_16_byte_aligned( joints );
    assert_16_byte_aligned( base );
...
============
*/
void VPCALL idSIMD_SSE2::TransformVertsAndTangents( idDrawVert *verts, const int numVerts, const idJointMat *joints, const idVec4 *base, const jointWeight_t *weights, const int numWeights )
{
...
============
*/
void VPCALL idSIMD_SSE2::TransformVertsAndTangentsFast( idDrawVert *verts, const int numVerts, const idJointMat *joints, const idVec4 *base, const jointWeight_t *weights, const int numWeights )
{
...
int VPCALL idSIMD_SSE2::ShadowVolume_CountFacingCull( byte *facing, const int numFaces, const vertIndex_t *indexes, const byte *cull )
{
    ...
    __asm {
        ...
        movzx		ecx, word ptr [esi+0*2]
        movzx		edx, byte ptr [edi+ecx]
        movzx		ecx, word ptr [esi+1*2]
        and			dl, byte ptr [edi+ecx]
        movzx		ecx, word ptr [esi+2*2]
        and			dl, byte ptr [edi+ecx]

        movzx		ecx, word ptr [esi+3*2]
        mov			dh, byte ptr [edi+ecx]
        movzx		ecx, word ptr [esi+4*2]
        and			dh, byte ptr [edi+ecx]
        movzx		ecx, word ptr [esi+5*2]
        and			dh, byte ptr [edi+ecx]
        movd		xmm0, edx

        movzx		ecx, word ptr [esi+6*2]
        movzx		edx, byte ptr [edi+ecx]
        movzx		ecx, word ptr [esi+7*2]
        and			dl, byte ptr [edi+ecx]
        movzx		ecx, word ptr [esi+8*2]
        and			dl, byte ptr [edi+ecx]

        movzx		ecx, word ptr [esi+9*2]
        mov			dh, byte ptr [edi+ecx]
        movzx		ecx, word ptr [esi+10*2]
        and			dh, byte ptr [edi+ecx]
        movzx		ecx, word ptr [esi+11*2]
        and			dh, byte ptr [edi+ecx]
        pinsrw		xmm0, edx, 1
...
int VPCALL idSIMD_SSE2::ShadowVolume_CreateSilTriangles( vertIndex_t *shadowIndexes, const byte *facing, const silEdge_t *silEdges, const int numSilEdges )
{
#if 1
    assert_sizeof( vertIndex_t, 4 );
...
/*
============
idSIMD_SSE2::ShadowVolume_CreateSilTrianglesParallel
============
*/
int VPCALL idSIMD_SSE2::ShadowVolume_CreateSilTrianglesParallel( vertIndex_t *shadowIndexes, const byte *facing, const silEdge_t *silEdges, const int numSilEdges )
{
#if defined ( GL_INDEX_INT )
    ...
    __asm {
        ...
        lea			edx, [ecx*2+ecx]
        add			edx, ebx
        movd		xmm2, dword ptr [edi+eax-4*16+8]
        movd		xmm3, dword ptr [edi+eax-4*16+12]
        movdqa		xmm4, xmm1
        movdqa		xmm5, xmm3
        psubd		xmm4, xmm0
        pand		xmm3, xmm4
        ...
        por			xmm3, xmm1
        por			xmm4, xmm0
        movd		dword ptr [ebx*4+0*4], xmm2
        movd		dword ptr [ebx*4+1*4], xmm4
        movd		dword ptr [ebx*4+2*4], xmm3
        ...
        lea			ebx, [ecx*2+ecx]
        add			ebx, edx
        movd		xmm2, dword ptr [edi+eax-3*16+8]
        movd		xmm3, dword ptr [edi+eax-3*16+12]
        movdqa		xmm4, xmm1
        movdqa		xmm5, xmm3
        psubd		xmm4, xmm0
        pand		xmm3, xmm4
        ...
        por			xmm3, xmm1
        por			xmm4, xmm0
        movd		dword ptr [edx*4+0*4], xmm2
        movd		dword ptr [edx*4+1*4], xmm4
        movd		dword ptr [edx*4+2*4], xmm3
        ...
        lea			edx, [ecx*2+ecx]
        add			edx, ebx
        movd		xmm2, dword ptr [edi+eax-2*16+8]
        movd		xmm3, dword ptr [edi+eax-2*16+12]
        movdqa		xmm4, xmm1
        movdqa		xmm5, xmm3
        psubd		xmm4, xmm0
        pand		xmm3, xmm4
        ...
        lea			ebx, [ecx*2+ecx]
        add			ebx, edx
        movd		xmm2, dword ptr [edi+eax-1*16+8]
        movd		xmm3, dword ptr [edi+eax-1*16+12]
        movdqa		xmm4, xmm1
        movdqa		xmm5, xmm3
        psubd		xmm4, xmm0
        pand		xmm3, xmm4
        ...
        lea			edx, [ecx*2+ecx]
        add			ebx, edx
        movd		xmm2, dword ptr [edi+eax+8]
        movd		xmm3, dword ptr [edi+eax+12]
        movdqa		xmm4, xmm1
        movdqa		xmm5, xmm3
        psubd		xmm4, xmm0
        pand		xmm3, xmm4
...
============
*/
int VPCALL idSIMD_SSE2::ShadowVolume_CreateCapTriangles( vertIndex_t *shadowIndexes, const byte *facing, const vertIndex_t *indexes, const int numIndexes )
{
#if defined ( GL_INDEX_INT )
    assert_sizeof( vertIndex_t, 4 );
...
/*
============
idSIMD_SSE2::ShadowVolume_CreateCapTrianglesParallel
============
*/
int VPCALL idSIMD_SSE2::ShadowVolume_CreateCapTrianglesParallel( vertIndex_t *shadowIndexes, const byte *facing, const vertIndex_t *indexes, const int numIndexes )
{
#if defined ( GL_INDEX_INT )
    ...
    __asm {
        ...
        loop4:
        movdqa		xmm0, [edi+edx*4-4*3*4+0]
        paddd		xmm0, xmm0
        pshufd		xmm2, xmm0, R_SHUFFLE_PS( 2, 2, 2, 2 )
        movzx		ecx, byte ptr [esi+eax-4]
        pshufd		xmm1, xmm0, R_SHUFFLE_PS( 1, 1, 1, 1 )
        sub			ecx, 1
        movd		dword ptr [ebx*4+0*4], xmm2
        and			ecx, 3
        movd		dword ptr [ebx*4+1*4], xmm1
        add			ecx, ebx
        movd		dword ptr [ebx*4+2*4], xmm0

        movdqa		xmm3, [edi+edx*4-3*3*4+4]
        paddd		xmm3, xmm3
        pshufd		xmm2, xmm0, R_SHUFFLE_PS( 3, 3, 3, 3 )
        movzx		ebx, byte ptr [esi+eax-3]
        pshufd		xmm1, xmm3, R_SHUFFLE_PS( 1, 1, 1, 1 )
        sub			ebx, 1
        movd		dword ptr [ecx*4+0*4], xmm1
        and			ebx, 3
        movd		dword ptr [ecx*4+1*4], xmm3
        add			ebx, ecx
        movd		dword ptr [ecx*4+2*4], xmm2

        movdqa		xmm4, [edi+edx*4-1*3*4-4]
        paddd		xmm4, xmm4
        pshufd		xmm1, xmm3, R_SHUFFLE_PS( 2, 2, 2, 2 )
        movzx		ecx, byte ptr [esi+eax-2]
        pshufd		xmm2, xmm3, R_SHUFFLE_PS( 3, 3, 3, 3 )
        sub			ecx, 1
        movd		dword ptr [ebx*4+0*4], xmm4
        and			ecx, 3
        movd		dword ptr [ebx*4+1*4], xmm2
        add			ecx, ebx
        movd		dword ptr [ebx*4+2*4], xmm1

        pshufd		xmm3, xmm4, R_SHUFFLE_PS( 3, 3, 3, 3 )
        pshufd		xmm2, xmm4, R_SHUFFLE_PS( 2, 2, 2, 2 )
        movzx		ebx, byte ptr [esi+eax-1]
        pshufd		xmm1, xmm4, R_SHUFFLE_PS( 1, 1, 1, 1 )
        sub			ebx, 1
        movd		dword ptr [ecx*4+0*4], xmm3
        and			ebx, 3
        movd		dword ptr [ecx*4+1*4], xmm2
        add			ebx, ecx
        movd		dword ptr [ecx*4+2*4], xmm1
