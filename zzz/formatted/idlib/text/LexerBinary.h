...
public:
    ...
    {
        return uniqueTokens.Allocated() + uniqueTokenHash.Allocated();
    }

    const idToken&			operator[]( int index ) const
    {
        return uniqueTokens[ index ];
    }
...
public:
    ...
    size_t					Allocated( void ) const
    {
        return tokens.Allocated() + tokenCache.Allocated();
    }
