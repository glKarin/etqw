...
public:
    ...
    bool		operator== ( const rvParticleParms &comp ) const
    {
        return( Compare( comp ) );
    }
    bool		operator!= ( const rvParticleParms &comp ) const
    ...
    void		Init( int spawnType = SPF_NONE_0 )
    {
    ...
    ID_INLINE void		Spawn( float *dest, const rvParticleParms &parms, idVec3 *normal, const idVec3 *centre )
    {
        ( *spawnFunctions[mSpawnType] )( dest, parms, normal, centre );
    }
    ID_INLINE void		Spawn( float *dest, const rvParticleParms &parms )
    {
        ( *spawnFunctions[mSpawnType] )( dest, parms, NULL, NULL );
    }
    void		HandleRelativeParms( float *death, float *init, int count );
    void		GetMinsMaxs( idVec3 &mins, idVec3 &maxs );

    ID_INLINE int GetFlags( void ) const
    {
        return mFlags;
    }
    ID_INLINE const idVec3 & GetMins( void ) const
    {
        return mMins;
    }
    ID_INLINE const idVec3 & GetMaxs( void ) const
    {
        return mMaxs;
    }
