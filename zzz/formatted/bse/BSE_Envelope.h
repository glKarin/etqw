...
public:
    ...
    rvEnvParms( const rvEnvParms &copy )
    {
        *this = copy;
    }
    ~rvEnvParms( void ) {}
    ...
    {
        mTable = declHolder.declTableType.LocalFind( type );
    }
    void		SetType( const idDeclTable *tabl )
    {
        mTable = tabl;
    }

    idVec3		&GetOffsetRef( void )
    {
        return( mEnvOffset );
    }
    const idVec3 &GetOffsetRef( void ) const
    {
        return( mEnvOffset );
    }
    idVec3		&GetRateRef( void )
    {
        return( mRate );
    }
    const idVec3 &GetRateRef( void ) const
    {
        return( mRate );
    }
    const idDeclTable *GetType( void ) const
    {
        return mTable;
    }
    ...
    bool		operator==	( const rvEnvParms &comp ) const
    {
        return( Compare( comp ) );
    }
    bool		operator!=	( const rvEnvParms &comp ) const
...
public:
    ...
    float		*GetStart( void )
    {
        return( mStart.ToFloatPtr() );
    }
    float		*GetEnd( void )
    {
        return( mEnd.ToFloatPtr() );
    }
