...
set OPTIONS=-DUSE_SYSTEM_CURL=ON -DTOOLS=ON -DUSE_SSE=OFF
...
if %BUILD_ARCH% == x86 (
    echo Platform: x86
    cmake -B ..\neo_win_x86_release -G "Visual Studio 17 2022" "-A Win32" -DCMAKE_TOOLCHAIN_FILE=%VCPKG_CMAKE_TOOLCHAIN% -DDIRECTXSDK_INCLUDE_PATH=%DIRECTXSDK_INCLUDE_PATH% %OPTIONS% CMakeLists.txt
) else (
    echo Platform: x86-64
    cmake -B ..\neo_win_x64_release -DCMAKE_TOOLCHAIN_FILE=%VCPKG_CMAKE_TOOLCHAIN% %OPTIONS% CMakeLists.txt
)

echo Build %BUILD_TYPE% ......
cmake --build ..\neo_win_%BUILD_ARCH%_release --config "%BUILD_TYPE%"

rem clean: cmake --build --config "%BUILD_TYPE%" --target clean

set TARGET_PATH=%cd%\..\neo_win_%BUILD_ARCH%_release\%BUILD_TYPE%
